...
const Button = forwardRef<HTMLButtonElement, IButtonProps>(
  (
    ...
    switch (variant) {
      case 'ghost':
        buttonColorClassNames = clsx('border-2 border-solid', colorClassNames[0], colorClassNames[2]);
        buttonDripColor = 'rgba(0, 0, 0, 0.1)';
        break;

      case 'transparent':
        buttonColorClassNames = clsx(
          colorClassNames[0],
          disabled || isLoading
            ? ''
      ...
      default:
        buttonColorClassNames = clsx(colorClassNames[1], colorClassNames[2]);
    ...
    return (
      <button
        ...
        {...buttonProps}
      >
        <span className={clsx(isLoading && 'invisible opacity-0')}>{children}</span>
